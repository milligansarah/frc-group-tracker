...
function App() {
  ...
  let teamAndYearRangePairs : TeamAndYearRangePairsType = {}
  for (const teamDataIndex in teams) {
    ...
    if (teamDataSeparated.length == 1) {
      teamAndYearRangePairs[teamDataSeparated[0]] = {}
    }
  ...
  return (
    <div className="App" style={{display: 'flex', backgroundColor: darkBlueBackgroundColor}}>
      <div style={{width: "fit-content", height: '100vh'}}>
        <InputPanelContentComponent teams={teamAndYearRangePairs} startYear={startYear} endYear={endYear}/>
      </div>
      <div style={{width: "100%", height: '100vh'}}>
        <OutputPanelContentComponent teams={teamAndYearRangePairs} startYear={startYear} endYear={endYear}/>
      </div>
    </div>
  );
}
